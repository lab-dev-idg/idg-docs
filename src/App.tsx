...
export default function App() {
  ...
  const currentSections = getSections(activeDoc, locale);
    ...
    <div className={`flex h-screen w-full bg-slate-50 text-slate-900 overflow-hidden ${isRTL ? 'font-kurdish text-right' : 'font-sans text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            {SPEC_KEYS.map((key) => {
              ...
              return (
                <button
                  ...
                  title={t(`title.${key}`)}
                  className={`px-1.5 py-1.5 rounded text-[10px] font-medium transition flex items-center justify-center gap-1 truncate cursor-pointer ${
                    isActive
        ...
        <div className="flex-1 p-3 overflow-hidden flex flex-col text-[11px] leading-tight">
          ...
          <nav className="flex-1 overflow-y-auto space-y-0.5 pr-1 text-slate-400 custom-scrollbar">
            <button
              ...
              className={`w-full text-start px-2.5 py-2 rounded transition flex items-center justify-between gap-2 cursor-pointer ${
                activeSection === null
            ...
            {filteredSections.map((sec) => (
              <button
                ...
                className={`w-full text-start px-2.5 py-1.5 rounded transition flex items-center justify-between gap-2 cursor-pointer ${
                  activeSection === sec
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-slate-50">
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 shadow-2xs">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-slate-500 text-xs font-mono">
              ...
              <span>/</span>
              <span className="text-slate-900 font-semibold" dir="ltr">{currentFile.split('/')[1]}</span>
            </div>
            <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded text-[10px] font-mono font-bold border border-indigo-200/80" dir="ltr">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleCopy}
              className="px-3 py-1.5 text-xs font-semibold text-slate-700 border border-slate-300 rounded-md hover:bg-slate-50 flex items-center gap-1.5 transition shadow-2xs cursor-pointer"
            >
        ...
        <section className="flex-1 flex overflow-hidden bg-white">
          {/* Line Numbers Gutter */}
          <div className={`w-12 bg-slate-50 flex flex-col items-center py-6 text-slate-400 font-mono text-[11px] leading-[1.8] select-none shrink-0 hidden sm:flex ${isRTL ? 'border-l border-slate-200' : 'border-r border-slate-200'}`} dir="ltr">
        ...
        <footer className="h-10 bg-slate-100 border-t border-slate-200 px-6 flex items-center justify-between text-[11px] text-slate-600 shrink-0 font-medium">
          ...
          <div className="flex items-center gap-4 text-slate-500 font-mono text-[10px]">
            <span dir="ltr">{t('footer.file_label')} {currentFile}</span>
            <span>{t('footer.status_label')}</span>
            <span>{t('footer.sync_label')}</span>
          </div>
        </footer>
      </main>
    </div>
